	...
	show: function(req, callback) {
		jwt.verify(req.cookies.token, jwt_key, function(err, data) {
			if (err)
				...
				var query = "SELECT *, HEX(id) as id, HEX(user_id) as user_id, HEX(trucker_id) as trucker_id FROM messages \
				WHERE HEX(application_id) = ? AND (HEX(user_id) = ? OR HEX(trucker_id) = ?) ORDER BY created_at";
				connection.query(query, [req.params.id, data.id, data.id], function(err, data) {
					if (err)
						callback({errors: {database: {message: "Please contact an admin."}}});
					else
						callback(false, data);
